...
:- use_module(chat80(nl/newdic), [word/1]).
:- use_module(chat80(nl/newerg), [sentence/5]).
:- use_module(chat80(nl/qplan), [qplan/2]).
:- use_module(chat80(nl/scopes), [clausify/2]).
:- use_module(chat80(nl/slots), [i_sentence/2]).
:- use_module(chat80(nl/talkr), [answer/2, dlst/3, write_tree/1]).

:- use_module(chat80(undefine), [pp_quant/2]).
